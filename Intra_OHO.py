...
def find_oh_o_hbonds(atom_positions, atom_types, cell, max_distance=3.5, min_distance=1.5, min_angle=120):
    """
    Identify OH--O intermolecular hydrogen bonds based on length and angle criteria.
    """
    hydrogen_bonds = []
    num_atoms = len(atom_types)

    for i, donor_type in enumerate(atom_types):
        if donor_type != 'O':  # Donor must be oxygen
            continue
        ...
        for j, hydrogen_type in enumerate(atom_types):
            ...
            o_h_distance = minimum_image_distance(atom_positions[i], atom_positions[j], cell)
            if o_h_distance > 1.1:  # Covalent bond threshold
                continue
...
def process_xyz_files(directory):
    """
    Process all .xyz files in the directory and allow user to select specific OH--O hydrogen bonds to track.
    """
    ...
    # Find OH--O hydrogen bonds in the first file
    hydrogen_bonds = find_oh_o_hbonds(atom_positions, atom_types, cell)

    print(f"\n--- Processing First File: {first_file} ---")
    print(f"  Detected {len(hydrogen_bonds)} OH--O hydrogen bonds:")
    for idx, hbond in enumerate(hydrogen_bonds):
        print(f"    {idx}: Donor {hbond['Donor']} - Hydrogen {hbond['Hydrogen']} - Acceptor {hbond['Acceptor']} "
              f"H---O Distance: {hbond['H---O Distance (Å)']:.3f}, O-H---O Angle: {hbond['O-H---O Angle (°)']:.2f}")

    # User selects indices to track
    selected_indices = input("Enter the indices of OH--O hydrogen bonds to track (comma-separated): ")
    ...
    for filename in sorted(xyz_files):
        ...
        angle_file = "OH--O_tracked_angle.txt"
        distance_file = "OH--O_tracked_distance.txt"

        with open(angle_file, 'a') as af, open(distance_file, 'a') as df:
            for idx in selected_indices:
                if idx < len(hydrogen_bonds):
                    hbond = hydrogen_bonds[idx]
                    af.write(f"{base_name}: {hbond['O-H---O Angle (°)']:.2f}\n")
